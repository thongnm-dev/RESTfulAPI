...
using RESTfulAPI.Models.CustomersParameters;
...
namespace RESTfulAPI.Controllers
{
	...
	public class CustomersController : BaseApiController
	{
			...
			Delta<CustomerDto> customerDelta)
		{
			...
			{
				await _genericAttributeService.SaveAttributeAsync(newCustomer, RESTfulAPICustomerDefaults.LanguageIdAttribute, languageId);
			}

			if (customerDelta.Dto.CurrencyId is int currencyId && await _currencyService.GetCurrencyByIdAsync(currencyId) != null)
			{
				await _genericAttributeService.SaveAttributeAsync(newCustomer, RESTfulAPICustomerDefaults.CurrencyIdAttribute, currencyId);
			}
			...
			Delta<CustomerDto> customerDelta)
		{
			...
			if (customerDelta.Dto.LanguageId is int languageId && await _languageService.GetLanguageByIdAsync(languageId) != null)
			{
				await _genericAttributeService.SaveAttributeAsync(currentCustomer, RESTfulAPICustomerDefaults.LanguageIdAttribute, languageId);
			}

			if (customerDelta.Dto.CurrencyId is int currencyId && await _currencyService.GetCurrencyByIdAsync(currencyId) != null)
			{
				await _genericAttributeService.SaveAttributeAsync(currentCustomer, RESTfulAPICustomerDefaults.CurrencyIdAttribute, currencyId);
			}
			...
			updatedCustomer.FirstName = await _genericAttributeService.GetAttributeAsync<string>(currentCustomer, RESTfulAPICustomerDefaults.FirstNameAttribute);
			updatedCustomer.LastName = await _genericAttributeService.GetAttributeAsync<string>(currentCustomer, RESTfulAPICustomerDefaults.LastNameAttribute);
			updatedCustomer.LanguageId = await _genericAttributeService.GetAttributeAsync<int>(currentCustomer, RESTfulAPICustomerDefaults.LanguageIdAttribute);
			updatedCustomer.CurrencyId = await _genericAttributeService.GetAttributeAsync<int>(currentCustomer, RESTfulAPICustomerDefaults.CurrencyIdAttribute);
		...
		private async Task<Address> InsertNewCustomerAddressIfDoesNotExist(Customer customer, AddressDto newAddress)
		{
			...
			var address = _addressService.FindAddress(allCustomerAddresses.ToList(),
				...
				newAddressEntity.County, newAddressEntity.StateProvinceId, 0, 0, newAddressEntity.ZipPostalCode,
		...
		private async Task InsertFirstAndLastNameGenericAttributesAsync(string firstName, string lastName, Customer newCustomer)
		{
			...
			if (firstName != null)
			{
				await _genericAttributeService.SaveAttributeAsync(newCustomer, RESTfulAPICustomerDefaults.FirstNameAttribute, firstName);
			}

			if (lastName != null)
			{
				await _genericAttributeService.SaveAttributeAsync(newCustomer, RESTfulAPICustomerDefaults.LastNameAttribute, lastName);
			}
		}
