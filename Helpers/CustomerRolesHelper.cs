...
namespace RESTfulAPI.Helpers
{
    ...
    {
        private const string CUSTOMERROLES_ALL_KEY = "RESTfulAPI.customerrole.all-{0}";
        ...
        {
            return customerRoles.FirstOrDefault(cr => cr.SystemName == RESTfulAPICustomerDefaults.GuestsRoleName) != null;
        }

        public bool IsInRegisteredRole(IList<CustomerRole> customerRoles)
        {
            return customerRoles.FirstOrDefault(cr => cr.SystemName == RESTfulAPICustomerDefaults.RegisteredRoleName) != null;
        }
