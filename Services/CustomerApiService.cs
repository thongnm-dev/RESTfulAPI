...
namespace RESTfulAPI.Services
{
    ...
    {
        private static readonly string FIRST_NAME = RESTfulAPICustomerDefaults.FirstNameAttribute.ToLowerInvariant();
        private static readonly string LAST_NAME = RESTfulAPICustomerDefaults.LastNameAttribute.ToLowerInvariant();
        private static readonly string LANGUAGE_ID = RESTfulAPICustomerDefaults.LanguageIdAttribute.ToLowerInvariant();
        private static readonly string CURRENCY_ID = RESTfulAPICustomerDefaults.CurrencyIdAttribute.ToLowerInvariant();
        private static readonly string DATE_OF_BIRTH = RESTfulAPICustomerDefaults.DateOfBirthAttribute.ToLowerInvariant();
        private static readonly string GENDER = RESTfulAPICustomerDefaults.GenderAttribute.ToLowerInvariant();
        private static readonly string VAT_NUMBER = RESTfulAPICustomerDefaults.VatNumberAttribute.ToLowerInvariant();
        private static readonly string VAT_NUMBER_STATUS_ID = RESTfulAPICustomerDefaults.VatNumberStatusIdAttribute.ToLowerInvariant();
        private static readonly string EU_COOKIE_LAW_ACCEPTED = RESTfulAPICustomerDefaults.EuCookieLawAcceptedAttribute.ToLowerInvariant();
        private static readonly string COMPANY = RESTfulAPICustomerDefaults.CompanyAttribute.ToLowerInvariant();
        ...
        private IQueryable<Customer> GetCustomersQuery(DateTime? createdAtMin = null, DateTime? createdAtMax = null, int sinceId = 0)
        {
            ...
            //                                                                           ccrm.CustomerRole.SystemName == RESTfulAPICustomerDefaults.GuestsRoleName)
        ...
        private async Task<int> GetDefaultStoreLangaugeIdAsync()
        {
            ...
            if (defaultLanguageId == 0)
            {
                ...
                // and use the one with the first display order. This is a default RESTfulAPICommerce workflow.
		...
		public async Task<Language> GetCustomerLanguageAsync(Customer customer)
		{
            ...
            var customerLanguageId = await _genericAttributeService.GetAttributeAsync<int>(customer, RESTfulAPICustomerDefaults.LanguageIdAttribute/*, store.Id*/);
		...
		{
            //var store = await _storeContext.GetCurrentStoreAsync();
            await _genericAttributeService.SaveAttributeAsync(customer, RESTfulAPICustomerDefaults.LanguageIdAttribute, language?.Id ?? 0/*, store.Id*/);
        }
		...
		public async Task<Currency> GetCustomerCurrencyAsync(Customer customer)
		{
            ...
            var customerCurrencyId = await _genericAttributeService.GetAttributeAsync<int>(customer, RESTfulAPICustomerDefaults.CurrencyIdAttribute/*, store.Id*/);
		...
		{
            //var store = await _storeContext.GetCurrentStoreAsync();
            await _genericAttributeService.SaveAttributeAsync(customer, RESTfulAPICustomerDefaults.CurrencyIdAttribute, currency?.Id ?? 0/*, store.Id*/);
        }
	}
}
