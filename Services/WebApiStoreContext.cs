...
using RESTfulAPI.Web.Framework;
...
namespace RESTfulAPI.Services
{
	public class WebApiStoreContext : IStoreContext
	{
		...
		public virtual async Task<int> GetActiveStoreScopeConfigurationAsync()
		{
			...
			if ((await _storeService.GetAllStoresAsync()).Count > 1)
			{
				...
				var storeId = await _genericAttributeService
					.GetAttributeAsync<int>(currentCustomer, RESTfulAPICustomerDefaults.AdminAreaStoreScopeConfigurationAttribute);
		...
		{
			string host = _httpContextAccessor.HttpContext?.Request.Headers["RESTfulAPIApiClientHost"]; // try read header provided by api client app
